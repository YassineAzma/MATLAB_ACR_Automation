...
line_prof_v = improfile(img_ACR(:,:,6),[centroid(1) centroid(1)],[1 size(img_ACR,2)]); % take a vertical line profile
line_prof_h = improfile(img_ACR(:,:,6),[1 size(img_ACR,1)],[centroid(2) centroid(2)]); % take a horizontal line profile

[~,locs_v] = findpeaks(line_prof_v,'MinPeakDistance',25/res_ACR(2),'NPeaks',1,'SortStr','descend'); % top to bottom
[~,locs_h] = findpeaks(line_prof_h,'MinPeakDistance',25/res_ACR(1),'NPeaks',1,'SortStr','descend'); % left to right
[~,locs_vr] = findpeaks(flip(line_prof_v),'MinPeakDistance',25/res_ACR(2),'NPeaks',1,'SortStr','descend'); % bottom to top
[~,locs_hr] = findpeaks(flip(line_prof_h),'MinPeakDistance',25/res_ACR(1),'NPeaks',1,'SortStr','descend'); % right to left
...
switch ind
    case 2
        ...
        corr_img_ACR = imrotate(img_ACR,-90,'bilinear','crop');
    case 3
        r_img_ACR = imrotate(img_ACR,-180,'bilinear','crop');
        corr_img_ACR = imrotate(img_ACR,-180,'bilinear','crop');
    case 4
        r_img_ACR = imrotate(img_ACR,-270,'bilinear','crop');
        corr_img_ACR = imrotate(img_ACR,-270,'bilinear','crop');
end
