...
function resolvable = ACR_SpatialResolution(img_ACR,obj_ACR,options)
%% Masking
...
for k = 1:size(img_ACR,4)
    ...
    centroid = ACR_Centroid(img_ACR(:,:,:,k),obj_ACR(k)); % find centroid

    mask = img_ACR(:,:,1,k).*ACR_Threshold(img_ACR(:,:,:,k),res_ACR,centroid); % threshold
    ...
    w_point = find(sum(cropped_mask,1)>0,1,'first')-1 + round(30/res_ACR(1)); % westmost point
    ...
    if strcmp(options.SuppressFigures,'no')
        figure
        imshow(img_ACR(round(centroid(2)):end,:,1,k),[],'InitialMagnification',200)
        hold on
