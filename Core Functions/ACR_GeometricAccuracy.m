...
function L = ACR_GeometricAccuracy(img_loc,img_ACR,obj_loc,obj_ACR,options)
...
for k = 1:size(img_ACR,4)
    rot_angle = ACR_FindRotation(img_ACR(:,:,:,k),obj_ACR(k));
    img_ACR(:,:,:,k) = ACR_RotateImages(img_ACR(:,:,:,k),rot_angle);
    res_ACR = ACR_RetrievePixelSpacing(obj_ACR(k));
    centroid = ACR_Centroid(img_ACR(:,:,:,k),obj_ACR(k)); % determine centroid

    mask_insert = ACR_Threshold(img_ACR(:,:,1,k),res_ACR,centroid);

    line_prof_v = improfile(mask_insert,[centroid(2) centroid(2)],[1 size(img_ACR,2)]); % take a vertical line profile
    line_prof_h = improfile(mask_insert,[1 size(img_ACR,1)],[centroid(1) centroid(1)]); % take a horizontal line profile
    ...
    mask_grid = ACR_Threshold(img_ACR(:,:,5,k),res_ACR,centroid);

    % Horizontal and Vertical
    line_prof_v = improfile(mask_grid,[centroid(1) centroid(1)],[1 size(img_ACR,2)]); % take a vertical line profile
    line_prof_h = improfile(mask_grid,[1 size(img_ACR,2)],[centroid(2) centroid(2)]); % take a horizontal line profile
    ...
    x_se = rot_matrix_se*[(1:size(img_ACR,2))-centroid(1);repmat(centroid(2),1,size(img_ACR,1))-centroid(2)];
    ...
    if ~isempty(img_loc)
        L(:,k) = round([loc_dist,insert_dist_v,insert_dist_h,grid_dist_v,grid_dist_h,grid_dist_se,grid_dist_sw],1); % output measured distances
    else
        L(:,k) = round([NaN,insert_dist_v,insert_dist_h,grid_dist_v,grid_dist_h,grid_dist_se,grid_dist_sw],1); % output measured distances without localiser
    end
end